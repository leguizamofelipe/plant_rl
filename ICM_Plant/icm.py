...
class ICM:
    def __init__(self, env, training_games=training_games, 
                 game_steps=game_steps, beta=0.01, lmd=0.99):
        self.env=env #import env
        self.state_shape=env.observation_space.shape # the state space
        self.action_shape=env.action_space.shape # the action space
        ...
        self.model=self.build_icm_model(state_shape = self.state_shape, action_shape=self.action_shape)
    ...
    def build_icm_model(self, state_shape=(2,), action_shape=(3,)):
        ...
        feature_vector_map=self.create_feature_vector(state_shape)
        fv_t=feature_vector_map(reshape(s_t))
        fv_t1=feature_vector_map(reshape(s_t1))

        a_t_hat=self.inverse_model(output_dim=action_shape[0])(fv_t, fv_t1)
    ...
    def learn(self, prev_states, states, actions, rewards):
        ...
        s_t=np.array(prev_states)
        s_t1=np.array(states)
    ...
    def batch_train(self):
        ...
        for game_index in range(self.training_games):
            ...
            state=self.env.reset() # each game is a new env
            game_reward=0
            running_reward=0
            print(game_index)
            for step_index in range(self.game_steps):
                ...
                int_r_state=np.reshape(state, (1,state.shape[0]))
                int_r_next_state=np.reshape(next_state, (1,next_state.shape[0]))
                int_r_action=np.reshape(action, (1,action.shape[0]))
    ...
    def act(self, current_state):
        ...
        # TODO Come back to this, not sure how to go from discrete to continuous here
        losses=[]
        actions = []
        for action_option in range(10):
            action = self.env.action_space.sample()

            actions.append(action)

            copy_env=copy.deepcopy(self.env)
            new_state, reward, _, _ = copy_env.step(action)
            ...
            loss=self.model.predict([np.array(current_state).reshape(-1,len(current_state)),
                                        ...
                                        np.array(action).reshape(-1,len(action)),
